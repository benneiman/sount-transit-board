...
secrets = {
    ...
    "aio_password": getenv("AIO_PASSWORD"),
...
wifi = adafruit_esp32spi_wifimanager.ESPSPI_WiFiManager(
    esp, secrets, status_light, debug=True
)
...
TRAIN_GROUPS_2 = (
    list(zip(STATION_CODES, config["train_groups_2"]))
...
def refresh_trains(train_groups: list) -> [dict]:
    try:
        trains = api.fetch_train_predictions(
            wifi, STATION_CODES, train_groups, WALKING_TIMES
        )
...
while True:
    if OFF_HOURS_ENABLED and is_off_hours():
        ...
        if config["swap_train_groups"]:
            train_groups = (
                TRAIN_GROUPS_1 if train_groups == TRAIN_GROUPS_2 else TRAIN_GROUPS_2
            )
